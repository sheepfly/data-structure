...
void ListStructureAscending(Node *list,int n){
    ...
    cur->next=list;//尾指针指向头结点
}
// 输出链表元素===========================================
void ListDisplay(Node list){
    // todo
    Node *cur=&list,*p=&list;
    while(cur->next){
        printf("%d\n",cur->next->data);
        cur=cur->next;
        if(cur->next->next==p->next)return;
    }
}
// 插入删除===============================================
...
// 合并两个有序链表=======================================
Node *ListMergeList(Node *list1,Node list2,int length){
    ...
    while(true){
        ...
        switch(tag){
            ...
            case 5:{
                ...
                printf("表2\n");
                ListDisplay(list2);
                ListMergeList(&list1,list2,length);
                printf("合并完成,结果如下\n");
                ListDisplay(list1);
                break;
            }
